"""
BTLx Process Classes for HOPS Generation

This module contains process classes that convert BTLx machining definitions
into HOPS (Holz-Her CNC) commands. Each class handles a specific type of
joint or machining operation.

Classes:
    FrenchRidgeProcess: Generates French ridge lap joint machining operations
    DoubleCutStepJointProcess: Generates double cut step joint operations
    DoubleCutProcess: Generates angled double cut operations with EBENEF planes
    TextProcess: Generates text engraving operations

Usage:
    from btlx_processes import FrenchRidgeProcess
    from writehops import HOPSWriter

    hopper = HOPSWriter(length=2000, width=60)
    process = FrenchRidgeProcess(hopper, face_front="11", ref_face=1)
...
class FrenchRidgeProcess:
    """
    Generates HOPS machining code for French ridge lap joints.

    A French ridge lap is a traditional timber joinery technique used in roof
    framing where two rafters meet at a ridge. This class generates the sawing
    and machining operations needed to create the joint.

    Attributes:
        face_front (str): Two-digit code indicating machining faces (00, 01, 10, 11, 12, 21, 22)
                         0 = None, 1 = Front, 2 = Back
        length (float): Beam length in mm
        width (float): Beam cross-section dimension in mm
        ref_face (int): Reference face ID (1-4)
        params (str): Generated HOPS command string
    """
...
class DoubleCutStepJointProcess:
    """
    Generates HOPS machining code for double cut step joints.

    A double cut step joint uses two angled cuts to create a stepped connection
    between timber members. This class generates SAEGEN (sawing) operations
    for creating the joint geometry.

    Attributes:
        orientation (str): "start" or "end" - where the joint is located
        angle1, angle2 (float): Cutting angles in degrees
        inclination1, inclination2 (float): Inclination angles in degrees
        ref_face (int): Reference face ID (1-4)
        startx, starty (float): Joint position coordinates
        params (str): Generated HOPS command string
    """
...
class DoubleCutProcess:
    """
    Generates HOPS machining code for angled double cut operations.

    Unlike DoubleCutStepJointProcess which uses EBENE0() and SAEGEN, this class
    uses EBENEF (custom work planes) with SP/G01/EP sequences for more complex
    angled cuts.

    Attributes:
        orientation (str): "start" or "end" - where the cut is located
        angle1, angle2 (float): Cutting angles in degrees
        inclination1, inclination2 (float): Inclination angles in degrees
        ref_face (int): Reference face ID (1-4)
        startx, starty (float): Cut position coordinates
        params (str): Generated HOPS command string
    """
...
class TextProcess:
    """
    Generates HOPS machining code for text engraving operations.

    Creates positioning frames for engraving text on timber surfaces.
    Typically used for marking beam numbers, assembly codes, or other
    identification markings.

    Attributes:
        ref_face (int): Reference face ID (1-4) where text will be engraved
        text (str): Text content to engrave
        startx, starty (float): Text position coordinates
        frame (Frame): Positioned frame for text engraving
    """
...
def wrap_to_pi(angle):
    """
    Normalize angle to [-π, π] range and convert to positive equivalent.

    Args:
        angle (float): Angle in radians

    Returns:
        float: Normalized angle in [0, π] range
    """
